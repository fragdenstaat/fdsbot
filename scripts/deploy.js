...
const collectChecks = (check_repos) =>
  new Promise(function (resolve, reject) {
    const promises = check_repos.map((path) =>
    ...
    return Promise.all(promises).then(function (results) {
      const checks = Array.from(results)
        .map((result) =>
          result.data.check_runs.filter((run) => /^(test|lint)/.test(run.name))
        )
        .flat()
      const pending = []
      const failed = []
...
var _runChecks = (pending_callback, resolve, reject, first) =>
  collectChecks(CHECK_REPOS).then(
    (checks) => resolve(checks),
    (bad_checks) => {
      if (bad_checks.failed.length > 0) {
        return reject(
          `Checks failed for: ${bad_checks.failed.map((f) => `<${f.url}|${f.repo}: ${f.name}>`).join(', ')}`
        )
...
const runChecks = (pending_callback) =>
...
module.exports = function (robot) {
  robot.receiveMiddleware(function (context, next, done) {
    ...
    if (!ALLOWED_USERS.includes(context.response.message.user.name)) {
      context.response.message.finish()
      ...
      if (
        context.response.message.text != null
          ? context.response.message.text.match(robot.respondPattern(''))
          : undefined
      ) {
        context.response.reply(
          `Hey ${context.response.message.user.name}, you are not allowed to talk to me! Please ask one of: ${ALLOWED_USERS.join(', ')}`
        )
      }
  ...
  const check_running = function (res) {
    ...
    if (deploying !== null) {
      const deploy_secs = Math.floor(
        (new Date().getTime() - deploying.time) / 1000
      )
      res.reply(
        `A deployment by ${deploying.user} for ${deploying.tags} is running for ${deploy_secs} seconds.`
      )
      return true
    }
    return false
  ...
  robot.respond(/deploy\s*$/, function (res) {
    ...
    return res.reply(
      `${res.message.user.name}, what should I deploy? Choose one of web, frontend, backend, all. Say: fdsbot deploy <tag>`
    )
  })
  ...
  robot.respond(/deploy (web|frontend|backend|all)/i, function (res) {
    ...
    return runChecks((pending) =>
      res.reply(
        'Checks are pending, deployment will continue automatically when checks pass.'
      )
    ).then(
      () => {
  ...
  robot.respond(/last deploy/, function (res) {
    ...
    if (last) {
      res.reply(
        `The last deployment was by ${last.user} from ${last.start} to ${last.end}.`
      )
      if (last.canceled) {
  ...
  robot.enter((res) =>
    res.send(
      `Hey @${res.message.user.name}! Willkommen im fragdenstaat-alerts-Channel!`
    )
  )
